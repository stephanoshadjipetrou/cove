    ...
    for activity in root.xpath(ACTIVITY_PREFIX):
        ...
        for org_type, orgs in orgs_in_data.items():
            for org in orgs:
                ...
                elif org_prefix in org_prefixes:
                    if org_prefix not in found_org_prefix:
                        found_org_prefix[org_prefix] = org_prefixes[org_prefix]
                        found_org_prefix[org_prefix]["orgs"] = set()
                        found_org_prefix[org_prefix]["count"] = 0
                        found_org_prefix[org_prefix]["type_count"] = org_type_template.copy()
                        found_org_prefix[org_prefix]["activity_ids"] = set()
                    found_org_prefix[org_prefix]["count"] += 1
                    found_org_prefix[org_prefix]["type_count"][org_type] += 1
                    found_org_prefix[org_prefix]["orgs"].add(org)
                    if iati_identifier:
                        found_org_prefix[org_prefix]["activity_ids"].add(iati_identifier)
